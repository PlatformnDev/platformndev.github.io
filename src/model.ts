...
export async function loadModel(progress:(done:number,total:number)=>void){
 ...
 try{const r=await fetch('/models/manifest.json');if(!r.ok)throw Error();manifest=await r.json();}catch{throw Error('MODEL_DOWNLOAD');}
 let cache:Cache|undefined;const cacheName='clearcut-model-'+manifest.revision;
 try{cache=await caches.open(cacheName);const names=await caches.keys();await Promise.all(names.filter(n=>n.startsWith('clearcut-model-')&&n!==cacheName).map(n=>caches.delete(n)));}catch{/* Cache is optional; never stores user data. */}
 const model=new Uint8Array(manifest.bytes);let offset=0;progress(0,manifest.bytes);
 for(const part of manifest.chunks){
 const url='/models/'+manifest.revision+'/'+part.file;let b:ArrayBuffer|undefined;
